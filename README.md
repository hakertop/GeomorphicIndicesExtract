...
公式如下：
$$ HI = \frac{H_{avg} - H_{min}}{H_{max} - H_{min}} $$
其中，$H_{min}$ 表示流域的最低点高程，$H_{max}$ 代表流域的最高点高程值，$O_{avg}$ 则是流域的平均高程值。
...
公式如下：
$$ Re = \frac{2(\frac{A}{\pi})}{L_b} $$
式中，$A$ 代表了流域的面积，$L_b$ 代表了流域从源头到河口的长度。
...
公式如下：
$$ BS = \frac{B_I}{B_W} $$
式中，$B_I$ 代表河流源头至出水口之间的平距，$B_W$ 表示与垂直方向的最大宽度。
...
公式如下：
$$ SL = (\frac{\Delta H}{\Delta L}) \times L $$
式中，$\Delta H$ 表示该段河流的高差，$\Delta L$ 表示该段河流的平距，$L$ 为该段河流距离源头的距离，那么 $frac{\Delta H}{\Delta L}$ 则表示为单位河流段的坡度。用均衡值 $K$ 对坡降指数（$SL$）值标准化处理得到 $SLK$ 指数，用以比较不同长度河道的活动性计算公式为：
$$ K = \frac{H_{total}}{ln(L_{total})} $$
$$ SLK = \frac{SL}{K} $$
其中，$H_{total}$ 表示河道总高差，$L_{total}$ 表示河道总长度。
将各段河流的 $SLK$ 值进行算术平均，可得到整个流域的 $SLK$ 的平均值。
...
公式如下：
$$ VF = \frac{2 V_{fw}}{(E_{ld} - E_{sc}) + (E_{rd} - E_{sc})} $$
式中，$V_{fw}$ 表示谷底宽度，$E_{ld}$ 表示左侧分水岭高程，$E_{rd}$ 表示右侧分水岭高程，$E_{sc}$ 表示谷底高程。
...
该函数集依赖于 TopoToolbox 库，请确保系统中已经安装了 TopoToolbox 库。
